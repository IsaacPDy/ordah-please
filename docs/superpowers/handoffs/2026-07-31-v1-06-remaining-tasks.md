...
- **Main worktree:** `.worktrees/V1-06-platform-admin-approval/` (off `main` at `c6d945e`)
...
Suggested agent split (per AGENTS.md worktree rules):

| Agent             | Task(s)           | Workstream branch                     | Worktree path                              |
| ----------------- | ----------------- | ------------------------------------- | ------------------------------------------ |
| Backend agent     | T4 then T5        | `workstream/V1-06-decide-runtime-api` | `.worktrees/V1-06-decide-runtime-api`      |
| Web UI agent      | T6                | `workstream/V1-06-decide-web-ui`      | `.worktrees/V1-06-decide-web-ui`           |
| Mobile UI agent   | T7                | `workstream/V1-06-decide-mobile-ui`   | `.worktrees/V1-06-decide-mobile-ui`        |
| Integration owner | T8 + final squash | (works on the task branch directly)   | `.worktrees/V1-06-platform-admin-approval` |

## Workflow rules (read before starting)

From `AGENTS.md` — non-negotiable:

1. **One agent, one writable worktree.** Never check out the same branch in two worktrees.
...
7. **`apps/web/.env.local` is gitignored.** Each new worktree must copy it from the main checkout: `cp /Users/fiona/Documents/Apps/Order\ App/apps/web/.env.local <worktree>/apps/web/.env.local`.
...
```bash
# 1. From the main repo, create your worktree off the current task branch tip
cd "/Users/fiona/Documents/Apps/Order App"
git worktree add .worktrees/<your-worktree-name> -b <your-branch-name> task/V1-06-platform-admin-approval

# 2. Enter the worktree (Claude Code: use EnterWorktree with path=...)
cd .worktrees/<your-worktree-name>

# 3. Install dependencies and copy the local env
npm ci
cp /Users/fiona/Documents/Apps/Order\ App/apps/web/.env.local apps/web/.env.local

# 4. Build shared packages so workspace exports resolve
npm run build --workspaces --if-present

# 5. Verify the baseline is clean (225 tests should pass before you start)
npm run test:unit
```
...
**This task runs in the main task worktree** (`.worktrees/V1-06-platform-admin-approval`), not a new workstream worktree.
...
1. **Integrate reviewed subtask branches one at a time:**
   ...
   ```bash
   cd "/Users/fiona/Documents/Apps/Order App/.worktrees/V1-06-platform-admin-approval"
...
8. **Cleanup:** Remove subtask worktrees and the task worktree only after the squash commit is verified on `main`. Per AGENTS.md, keep the task branch as a recovery reference until the squash is verified and pushed.

## Handoff prompts

Copy-paste one of these into a new Claude Code session to start an agent on a specific task. The agent should invoke `superpowers:test-driven-development` (and `superpowers:using-git-worktrees` if it needs to set up its own worktree).
...
Setup: create worktree .worktrees/V1-06-decide-runtime-api on branch workstream/V1-06-decide-runtime-api off task/V1-06-platform-admin-approval. Copy apps/web/.env.local from the main checkout. npm ci, build packages.
...
Setup: create worktree .worktrees/V1-06-decide-web-ui on branch workstream/V1-06-decide-web-ui off task/V1-06-platform-admin-approval. Copy apps/web/.env.local from the main checkout. npm ci, build packages.
...
Setup: create worktree .worktrees/V1-06-decide-mobile-ui on branch workstream/V1-06-decide-mobile-ui off task/V1-06-platform-admin-approval. npm ci, build packages.
...
Read in order:
1. AGENTS.md (especially the worktree-and-permanent-history rules)
2. docs/superpowers/handoffs/2026-07-31-v1-06-remaining-tasks.md (this handoff)
3. context/progress-tracker.md (current state)

You will work in the existing .worktrees/V1-06-platform-admin-approval worktree on branch task/V1-06-platform-admin-approval. The completed workstream branches workstream/V1-06-decide-runtime-api, workstream/V1-06-decide-web-ui, and workstream/V1-06-decide-mobile-ui should already be pushed.

Integrate them one at a time with affected checks after each merge. Update context/progress-tracker.md (V1-06 done, V1-06B added). Run the complete verification matrix from the "Task 8" brief. Run live web and mobile acceptance against a real platform-admin session. Squash-merge to main with the exact title "V1-06 Implement platform-admin approval and limited mobile-admin permissions". Verify main, push, and only then clean up subtask worktrees.
```
