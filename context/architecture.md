...
- Each group has one persistent, multi-use, deployment-bound invite link, minted or rotated by a Group Owner or Platform Admin. Neon stores only the link's hash and a short non-secret prefix so a database read cannot reveal a usable link.
- Invite-link acceptance is idempotent: it creates or reactivates one membership per user per group in a transaction and never consumes the link. A rotated link stops working immediately. A user may hold multiple active memberships with a different role in each group, while acceptance never enrolls the member in an order.
- The original single-use `invitations` table is preserved read-only as audit history; new acceptances honor only the persistent-link model.
