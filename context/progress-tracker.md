...
- Documentation-first migration work is complete: permanent architecture, UI, service, environment, coding, workflow, product-design, implementation-plan, and service-limit records now describe Better Auth and the Google OAuth boundary. Signed-in browser/device acceptance and final integration remain.
...
- Google OAuth is External and Testing with only `openid`, email, and profile scopes, and the localhost plus production redirect URIs are registered. Signed-in acceptance remains blocked because the Google test-user list is empty; an approved account must be added without sharing its address in chat.
