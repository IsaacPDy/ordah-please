...
- Clerk has no active webhook endpoint and no Vercel or local runtime variables. The external Clerk development application was deleted on 2026-07-30 as the final destructive cleanup once V1-04A was permanently squash-committed on `main` and the rollback rationale expired; only historical references in generated migrations, anti-regression tests, and session evidence remain.
...
- V1-04A Google credentials were rotated after a diagnostic surface exposed the previous client secret. The ignored local environment and Vercel Preview now use the replacement, a fresh localhost callback passed, the old Google secret was disabled and deleted, and only one enabled secret remains.
- The Clerk development application was deleted on 2026-07-30 after V1-04A was permanently squash-committed on `main` and Better Auth acceptance completed; the rollback rationale expired, so the destructive cleanup was completed. All active Clerk packages, code, Vercel variables, and local variables were already retired.
