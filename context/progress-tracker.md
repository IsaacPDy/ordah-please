...
- Rollback-only Neon verification passes all four repository provider tests, including concurrent first-request provisioning. Persistent migration is blocked safely: the configured local database URLs resolve to the sole Neon branch named `production`, and Neon rejected connected attempts to create both a named and unnamed development branch with `INVALID_ARGUMENT`. No production migration was attempted.
- External setup remains required before acceptance: create an isolated Neon development branch and provide its pooled/direct URLs locally, then configure Google OAuth plus environment-specific Better Auth secrets and URLs. Secret values remain outside Git and must not be sent in chat.
