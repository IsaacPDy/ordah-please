...
- Member web/PWA layout-reference screenshot: `design-qa-home-390x844.png` (saved before the accessibility follow-up; not used as current color evidence)
- Admin web layout-reference screenshot: `design-qa-admin-1440x900.png` (saved before the accessibility follow-up; not used as current color evidence)
...
- The current member Home shell was reopened and inspected live at 390 by 844 after the accessibility follow-up. The committed screenshot remains an earlier layout-reference artifact and is not presented as a post-fix capture.
...
- The current admin overview was reopened and inspected live at 1440 by 900. It had one `h1`, no horizontal overflow, and six 215-by-44 CSS-pixel navigation targets. The committed screenshot remains an earlier layout-reference artifact.
...
The approved reference and saved `design-qa-home-390x844.png` layout reference were compared at a matching mobile aspect ratio. The scoped regions preserve the reference hierarchy: green `ordah please` identity on a white header, bright light canvas, pale-mint rounded information card, charcoal headings, restrained border/shadow treatment, outlined navigation icons, and a mint selected-navigation surface. The empty state deliberately contains less content than the complete reference because restaurant and active-order features are not part of Task 0.3. Current color claims below come from live browser-computed values and regression tests, not from the saved pre-follow-up image.

The saved desktop admin layout reference confirms that the token language scales into a denser table-ready layout without copying the member bottom-navigation structure. Current structure and target-size claims come from the live post-fix browser inspection.
...
- No scoped P0, P1, or P2 web/PWA fidelity issue remains after the live post-fix inspection and automated color regression checks.
- The committed screenshots are retained only as pre-follow-up layout references. They are not claimed as current post-fix pixel evidence.
...
- Pass 4: the root task reopened the post-fix member and admin shells and rechecked current navigation, active state, keyboard focus, target dimensions, overflow, heading structure, computed tokens, and browser logs. The previously committed images remained unchanged and are therefore documented only as layout references, not as post-fix captures. No scoped P0/P1/P2 web/PWA issue remained.

## Implementation Checklist

- [x] Reopen and inspect the current member web/PWA Home shell at 390 by 844; keep the earlier image explicitly labeled as a layout reference.
- [x] Reopen and inspect the current desktop admin overview at 1440 by 900; keep the earlier image explicitly labeled as a layout reference.
